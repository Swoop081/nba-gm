import * as base from './season-engine.js?v=0.6.159';
import {NBA_CUP_2026_GROUP_PLAY} from '../data/nba-cup-2026-group-play.js';
export * from './season-engine.js?v=0.6.159';
...
const TOR_OPENING=[['2026-10-21','CHI','TOR'],['2026-10-23','TOR','WAS'],['2026-10-25','TOR','MIN'],['2026-10-28','ORL','TOR'],['2026-10-30','ORL','TOR']];
function enforceTorontoOpeningSchedule(season){if(!season?.schedule)return season;for(const [date,away,home] of TOR_OPENING){season.schedule=season.schedule.filter(g=>!(g.date===date&&([away,home].includes(g.home)||[away,home].includes(g.away))));season.schedule.push({date,tipUtc:`${date}T23:30:00Z`,away,home,venue:'',played:false,official:true});}season.schedule.sort((a,b)=>a.date.localeCompare(b.date)||(a.tipUtc||'').localeCompare(b.tipUtc||''));season.schedule.forEach((g,i)=>g.id=i+1);return season}
...
export function loadSeason(){const s=base.loadSeason();return s?applyOfficialCupGroupSchedule(enforceTorontoOpeningSchedule(s)):null}
export async function createSeason(){return applyOfficialCupGroupSchedule(enforceTorontoOpeningSchedule(await base.createSeason()))}
