...
export function nextTeamGame(season=loadSeason(),team){return season?.schedule.find(g=>!g.played&&(g.home===team||g.away===team))||null}
export async function simulateToNextTeamGame(team){let s=loadSeason()||await createSeason();const target=nextTeamGame(s,team);if(!target)return s;for(const g of s.schedule){if(g.played)continue;s=await simulateScheduledGame(s,g.id);if(g.id===target.id)break}return s}
export async function simulateTeamGames(team,count=1){let s=loadSeason()||await createSeason();for(let i=0;i<count;i++){const target=nextTeamGame(s,team);if(!target)break;s=await simulateToNextTeamGame(team)}return s}
